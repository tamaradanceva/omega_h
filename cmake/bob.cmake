function(get_cpu_arch arch)
  if (${PROJECT_NAME}_CPU_ARCH)
    set(${arch} "${${PROJECT_NAME}_CPU_ARCH}" PARENT_SCOPE)
    return()
  endif()
  set(architectures
    Broadwell
  ...
  if (CMAKE_SYSTEM_NAME STREQUAL "Linux")
    execute_process(COMMAND
      bash "-c" "cat /proc/cpuinfo | grep 'model name' | uniq"
      OUTPUT_VARIABLE model_name)
    string(REPLACE "\n" "" model_name "${model_name}")
    string(FIND "${model_name}" ":" colon_pos)
    math(EXPR name_start "${colon_pos} + 2")
    string(SUBSTRING "${model_name}" ${name_start} -1 model_name)
    message(STATUS "CPU model name: \"${model_name}\"")
    set(Broadwell_CPUs
      "Intel(R) Xeon(R) CPU E5-2630 v4 @ 2.20GHz"
      )
    foreach (possible_arch IN LISTS architectures)
      list(FIND ${possible_arch}_CPUs "${model_name}" index)
      if (NOT index EQUAL "-1")
        set(${arch} "${possible_arch}" PARENT_SCOPE)
        return()
      endif()
    endforeach()
    message(WARNING "CPU model name \"${model_name}\" not found in known lists, please add it to bob.cmake")
  endif()
endfunction(get_cpu_arch)

function(get_hardware_flags hw_flags)
  get_cpu_arch(cpu_arch)
  message(STATUS "CPU (micro)architecture: \"${cpu_arch}\"")
  if (CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
    set(Westmere_GNU_arch "westmere")
    set(Broadwell_GNU_arch "broadwell")
    if (NOT ${cpu_arch}_GNU_arch)
      message(WARNING "No known GCC -march argument for CPU (micro)architecture \"${cpu_arch}\", please add it to bob.cmake")
    endif()
    set(${hw_flags} "-march=${${cpu_arch}_GNU_arch}" PARENT_SCOPE)
    return()
  endif()
endfunction(get_hardware_flags)
...
function(bob_begin_cxx_flags)
  if (${PROJECT_NAME}_NORMAL_CXX_FLAGS)
    ...
    if(${PROJECT_NAME}_CXX_OPTIMIZE)
      set(FLAGS "${FLAGS} -O3")
      get_hardware_flags(HW_FLAGS)
      set(FLAGS "${FLAGS} ${HW_FLAGS}")
